package lse.math.games.builder.viewmodel.action 
{
	...
	/**	
	 * Adds children to all the nodes in a selected iset/ in a selected node's iset, putting them
	 * inside an existing Iset, if possible, else on a new Iset.
	 ...
	 * @author Mark Egesdal
	 */
	public class AddChildIsetAction implements IAction
	{
		...
		public function AddChildIsetAction(iset:Iset, node:Node) 
		{
		...
		{			
			...
			if (iset != null) {
				...
				if (node != null) {
					...
					log.add(Log.ERROR, "Couldn't find any node with idx "+_nodeId, "AddChildIsetAction");
			} else
				log.add(Log.ERROR, "Couldn't find any iset with idx "+_isetId, "AddChildIsetAction");
